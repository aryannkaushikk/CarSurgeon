  ...
  return (
    <div id="welcome">
      <button
        ...
        style={{
          color: buttonPressed ? "black" : isHovering ? "red" : "white",
        }}
        onMouseEnter={() => hovering()}
        onMouseLeave={() => hovering()}
      >
        {buttonPressed ? "Close" : "Menu"}
      </button>
      ...
      >
        <h1 style={infoH1}>
          {isAnimating ? (
            <NumberFlow value={Math.ceil((number * 8) / 100) + number} />
          ) : (
            <NumberFlow value={11} />
          )}
          +
      ...
      >
        <h1 style={infoH1}>
          {isAnimating ? (
            <NumberFlow
              value={Math.ceil(
                (number * 8 * 1000 + number * 7 * 10 + number) / 1000
              )}
            />
          ) : (
